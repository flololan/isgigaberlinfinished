...
<footer
  class="d-flex flex-column align-items-center mt-5 mx-0 py-4 px-1 px-md-5 text-white"
>
  ...
  <p class="mb-0 ms-2 text-center fw-light">
    ...
    {#each DEVS as { name, link }, index}
      <a href={link} target="_blank" class="link-light">{name}</a
      >{#if index < DEVS.length - 1},&nbsp;{/if}
    {/each}
  </p>
